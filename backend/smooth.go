	...
	app := &cli.App{
		...
		Flags: []cli.Flag{
			&cli.StringFlag{
				...
				Value: "/home/wsl-ubuntu/blockchain/filecoin/onramp-contracts/contract-tools/xchain/config.json",
			},
		},
	...
	for {
		...
		case latestEvent := <-a.ch:
			...
			// if err != nil { // we've overshot, lets commit to just pieces in pending
				...
				fmt.Println("len pieces", len(pieces))
				...
				aggLocation := `/home/wsl-ubuntu/blockchain/filecoin/onramp-contracts/contract-tools/aggregate/` + aggCommp.String()
...
func (a *aggregator) sendDeal(ctx context.Context, aggCommp cid.Cid, url string) error {
	...
	fmt.Println(dealParams)
	fmt.Println(dealParams.ClientDealProposal.Proposal.Label)
	if !resp.Accepted {
...
func loadPrivateKey(cfg *Config) (*bind.TransactOpts, error) {
	...
	// Import existing key
	a, err := ks.Import(keyJSON, "testing", "testing")
	if err != nil {
		return nil, fmt.Errorf("failed to import key %s: %w", cfg.ClientAddr, err)
	}
	if err := ks.Unlock(a, "testing"); err != nil {
